  ...
  private _normaliseReviewModel(review: any) {
    ...
    private _setupContentsObserver() {
      ...
      if (!this.showContents) {
        ...
        this._isBelowContents = false;
        this._isFooterInViewport = false;
      ...
      requestAnimationFrame(() => {
        ...
        this._contentsObserver = new IntersectionObserver(
          ([entry]) => {
            if (entry.isIntersecting) {
              ...
              this._isBelowContents = entry.boundingClientRect.top < 0;
            }
            this._syncFloatingBackToContents();
            this._cdr.detectChanges();
          },
          {
            root: null,
        ...
        if (footerEl) {
          this._footerObserver = new IntersectionObserver(
            ([entry]) => {
              this._isFooterInViewport = entry.isIntersecting;
              this._syncFloatingBackToContents();
              this._cdr.detectChanges();
            },
            {
          ...
          this._footerObserver.observe(footerEl);
        }
      });
    }
    ...
    private _teardownFooterObserver() {
      this._footerObserver?.disconnect();
      this._footerObserver = undefined;
    }

    private _syncFloatingBackToContents() {
      this.showFloatingBackToContents = this._hasSeenContentsInViewport && this._isBelowContents && !this._isFooterInViewport;
    }
